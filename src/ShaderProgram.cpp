...
GLint ShaderProgram::getAttribLocation(const GLchar* attribute) {
	GLCall(GLint posAttrib = glGetAttribLocation(m_ShaderProgramID, attribute));
	return posAttrib;
}

void ShaderProgram::setLayout(const GLchar* attribName, GLint size, GLenum type, GLboolean normalized, GLsizei stride, const GLvoid* pointer) {
	GLCall(GLint index = glGetAttribLocation(m_ShaderProgramID, attribName));
